...
class JobsModule extends Module {
    ...
    public function __construct() {
		...
		public function home() {
			...
			return $tpl->render(array("jobs" => $records["records"], "isAdmin" => isAdminUser($user, $admin), 
		...
		public function deletePosting() {
			...
			$obj = $force->deleteRecordFromSession("Job__c", $body->Id);

			//returning http response status 302 returns to homepage 
			header('Location: /jobs', true, 302);

			return */ //returned in create/update -> $obj["records"][0]["Id"];
		}
	...
	//		var_dump($body);
			...
			if($body->Id == "") {
				...
				$obj = $force->updateRecordsFromSession($req->getBody(), "Job__c");
			}
